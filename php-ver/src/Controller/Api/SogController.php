...
class SogController extends AbstractController
{
    ...
    {   
	    // TODO: netoyer cela d'ici.
        //$ip = $request->getClientIp();

        //if ($ip != "37.64.127.218" && $ip != "82.67.61.9" && $ip != "102.68.192.186" && $ip != "176.190.163.155") {
    ...
    // TODO: Cette fonction a une raison d'exister encore?
    /**
    ...
    { 
        ...
        // PROD. (elle sera changé au release)
    ...
    /**
     * @Route("/paiement-check-ipn", name="paiement_check_ipn")
     */
    public function validerIPN(Request $request): Response
    {
        $data = $request->request->all();

        $shaKey = 'prodpassword_UDcmgIlcQJr8nsOPy2c38LEZkmm4kgvlNR8hj99D8wS0j';
        if (!$this->checkHash($data, $shaKey)) {
        ...
        // TODO: Freudiciot, stp vérifie que c'est correcte.
        // $montant = ((float)$json['orderDetails']['orderTotalAmount']) / 100;
        // $_boutique = $this->boutRepository->findOneBy(['user' => $this->getUser()]);
        // $_credit = new Credit();
    ...
    { 
        ...
        // Côté client on valide avec le SHA, cote IPN avec le clé API.	
        // PROD
        $shaKey = 'BXhwZJOT9oOgla0BeEnyuOcJyfjLiiuriBmiaRpxeuom5';
        // TEST
        //$shaKey = 'TA2TsYbhFBgapy20Bte24O2ywcJGFZ6cNVmhRvvu8YolN';
        if (!$this->checkHash($data, $shaKey)) {
            // Redirigez ou renvoyez une réponse avec un message d'erreur.
